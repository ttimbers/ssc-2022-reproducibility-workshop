...
knitr::include_graphics("img/05-shareable-compute/renv-logo.png")
```
...
1. Click the green "Use this template" button in [this repository](TBD) to obtain a copy of it for yourself.
...
- Install the `fortunes` R package via:

```{r install-fortunes, echo=TRUE, eval=FALSE}
install.packages("fortunes")
```

- Add the following code to a code chunk at the top of your `report.rmd`
...
You should see something like this near the end of the file:
...
3. Examine `report.rmd` to see what packages the project depends on.

4. Call `renv::snapshot()` to save the state of the project library to the lockfile (called `renv.lock`).

5. Put the `.Rprofile`, `renv.lock` and `renv/activate.R` files under local 
and remote version control.

Examine the `renv.lock` file - do the packages there match what you saw at the top of `report.rmd` - that's neat... How does that work???
